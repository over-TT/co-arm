...
## When adding more build files

1. Add label photographs and use them to finish `bom/bom.csv`.
2. Put new printed parts in `3mf/` with the same clean naming style.
3. Add editable CAD to `cad/native/`.
4. Export the matching revision to `cad/step/`.
5. Export printable parts to `stl/`; promote only checked parts to
   `stl/print-ready/`.
6. Add revision-matched drawings to `drawings/`.
7. Add electrical source files and readable exports to
   `electronics/schematics/` and `electronics/wiring/`.
8. Update the assembly documentation only after checking the files against the
   physical arm.
